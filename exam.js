...
// @version      1.0.0
// @description  利用Report页面获取答案自动填充，并支持题库累积、导入、多格式导出、清空
// @author       Keggin
...
(function() {
    ...
    if (isReportPage) {
        console.log('检测到Report页面，准备提取答案(v1.8)...');
        addExtractButton();
    ...
    if (isDefaultPage) {
        ...
        panel.style.cssText = `position: fixed; top: 100px; right: 20px; z-index: 9998; background: white; border: 1px solid #ccc; border-radius: 8px; padding: 15px; box-shadow: 0 4px 10px rgba(0,0,0,0.2); display: flex; flex-direction: column; gap: 10px;`;
        ...
        importButton.onclick = () => document.getElementById('gm-importer-panel').style.display = 'block';
